...
function ResetPasswordForm() {
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
        ...
        <InputField label="New Password" icon={Lock} type="password" placeholder="••••••••" {...register('password')} error={errors.password?.message} />
        ...
        <div className="w-full lg:w-1/2 min-h-screen flex items-center justify-center p-6 lg:p-12 relative overflow-y-auto">
             ...
             >
                <div className="mb-10 text-left">
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight mb-2">Set New Password</h1>
                    <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed max-w-[380px]">
                        Join the SLT Digital professional network and secure your business identity with a new password today.
                    </p>
                </div>

                <Suspense fallback={<div className="text-center py-10"><Loader2 className="w-8 h-8 animate-spin mx-auto text-[#00B4EB]" /></div>}>
                    <ResetPasswordForm />
                </Suspense>
